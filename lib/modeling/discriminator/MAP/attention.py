...
class AttentionConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('k_out_U',k_out.unfold(2, self.kernel_size, self.stride).size())
        k_out = k_out.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride)
        # print('k_out_U',k_out.size())
        v_out = v_out.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride)
        # print('v_out_U',v_out.size())

        k_out_h, k_out_w = k_out.split(self.out_channels // 2, dim=1)
        # print('k_out_h',k_out_h.size())
        # print('k_out_w',k_out_w.size())
        # print('k_out_h + self.rel_h',(k_out_h + self.rel_h).size())
        k_out = torch.cat((k_out_h + self.rel_h, k_out_w + self.rel_w), dim=1)
        # print('k_out_C',k_out.size())
        ...
        # print('out',torch.einsum('bnchwk,bnchwk -> bnchw', out, v_out).size())
