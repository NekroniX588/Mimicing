...
class SSDLite(nn.Module):
    ...
    def __init__(self, base, extras, head, feature_layer, num_classes):
        ...
        self.transform0 = nn.Conv2d(160, 512, kernel_size = 1)
        self.transform1 = nn.Conv2d(960, 1024, kernel_size = 1)

        self.transform5 = nn.Conv2d(128, 256, kernel_size = 1)
    ...
    def forward(self, x, phase='eval'):
        ...
        x = self.base.bn1(x)
        x = self.base.hs1(x)
        x = self.base.bneck[:13](x)
        s = self.norm(x)
        sources.append(s)
        x = self.base.bneck[13:15](x)
        x = self.base.conv2(x)
        x = self.base.bn2(x)
        x = self.base.hs2(x)
        sources.append(x)
        ...
        if phase == 'train_mimic':
            FM = []
            FM.append(self.transform0(sources[0]))
            FM.append(self.transform1(sources[1]))
            FM += sources[2:5]
            FM.append(self.transform5(sources[5]))
            return output, FM
        else:
            return output
